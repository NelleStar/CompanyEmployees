    ...
    public class Entity : DynamicObject, IXmlSerializable, IDictionary<string, object>
    {
        ...
        private void WriteLinksToXml(string key, object value, XmlWriter writer)
        {
            ...
            if (value is List<Link> links)
            {
                ...
                {
                    writer.WriteStartElement(nameof(Link));
                    WriteLinksToXml(nameof(link.Href), link.Href ?? string.Empty, writer);
                    WriteLinksToXml(nameof(link.Method), link.Method ?? string.Empty, writer);
                    WriteLinksToXml(nameof(link.Rel), link.Rel ?? string.Empty, writer);
                    writer.WriteEndElement();
                }
            }
            else
